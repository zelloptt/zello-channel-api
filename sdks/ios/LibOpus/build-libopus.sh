...
OUTPUT_HEADERS_PATH="/CSource/include/"
OUTPUT_LIB_PATH="/../Zello/"
...
do
    if [ "${ARCH}" == "i386" ] || [ "${ARCH}" == "x86_64" ]; then
        ...
        EXTRA_CONFIG="--enable-float-approx"
    else
        PLATFORM="iPhoneOS"
        EXTRA_CFLAGS="-arch ${ARCH}"
        EXTRA_CONFIG="--host=arm-apple-darwin"
        # fixed point build breaks for arm64 so use float point there
        if [ "${ARCH}" == "arm64" ]; then
			EXTRA_CONFIG="--enable-float-approx ${EXTRA_CONFIG}"
		else
			# don't include these as the difference is not perceivable
			# --with-NE10-libraries=${REPOROOT}/ne10 --with-NE10-includes=${REPOROOT}/ne10
			EXTRA_CONFIG="--enable-fixed-point --enable-intrinsics ${EXTRA_CONFIG}"
		fi
    fi
...
for OUTPUT_LIB in ${OUTPUT_LIBS}; do
	...
	if [ -n "$INPUT_LIBS"  ]; then
		...
		-output "${REPOROOT}${OUTPUT_LIB_PATH}${OUTPUT_LIB}"
	else
...
for ARCH in ${ARCHS}; do
	...
	cp -R ${INTERDIR}/${PLATFORM}${SDKVERSION}-${ARCH}.sdk/include/* ${REPOROOT}${OUTPUT_HEADERS_PATH}
	if [ $? == "0" ]; then
