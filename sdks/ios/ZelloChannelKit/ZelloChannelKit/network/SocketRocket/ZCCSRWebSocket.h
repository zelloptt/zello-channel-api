...
typedef NS_ENUM(NSInteger, ZCCSRReadyState) {
    SR_CONNECTING   = 0,
    SR_OPEN         = 1,
    SR_CLOSING      = 2,
    SR_CLOSED       = 3,
...
typedef NS_ENUM(NSInteger, ZCCSRStatusCode) {
    // 0-999: Reserved and not used.
    ZCCSRStatusCodeNormal = 1000,
    ZCCSRStatusCodeGoingAway = 1001,
    ZCCSRStatusCodeProtocolError = 1002,
    ZCCSRStatusCodeUnhandledType = 1003,
    // 1004 reserved.
    ZCCSRStatusNoStatusReceived = 1005,
    ZCCSRStatusCodeAbnormal = 1006,
    ZCCSRStatusCodeInvalidUTF8 = 1007,
    ZCCSRStatusCodePolicyViolated = 1008,
    ZCCSRStatusCodeMessageTooBig = 1009,
    ZCCSRStatusCodeMissingExtension = 1010,
    ZCCSRStatusCodeInternalError = 1011,
    ZCCSRStatusCodeServiceRestart = 1012,
    ZCCSRStatusCodeTryAgainLater = 1013,
    // 1014: Reserved for future use by the WebSocket standard.
    ZCCSRStatusCodeTLSHandshake = 1015,
...
@class ZCCSRSecurityPolicy;

/**
 Error domain used for errors reported by ZCCSRWebSocket.
 */
extern NSString *const ZCCSRWebSocketErrorDomain;

/**
 Key used for HTTP status code if bad response was received from the server.
 */
extern NSString *const ZCCSRHTTPResponseErrorKey;

@protocol ZCCSRWebSocketDelegate;

///--------------------------------------
#pragma mark - ZCCSRWebSocket
///--------------------------------------

/**
 A `ZCCSRWebSocket` object lets you connect, send and receive data to a remote Web Socket.
 */
@interface ZCCSRWebSocket : NSObject <NSStreamDelegate>
 ...
 */
@property (atomic, assign, readonly) ZCCSRReadyState readyState;
...
/**
 ...
 @param securityPolicy Policy object describing transport security behavior.
 */
- (instancetype)initWithURLRequest:(NSURLRequest *)request protocols:(nullable NSArray<NSString *> *)protocols securityPolicy:(ZCCSRSecurityPolicy *)securityPolicy NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 By default, a web socket will schedule itself on `+[NSRunLoop ZCCSR_networkRunLoop]` using `NSDefaultRunLoopMode`.
...
/**
 ...
 An instance of `ZCCSRWebSocket` is intended for one-time-use only. This method should be called once and only once.
 */
- (void)open;

/**
 Closes a web socket using `ZCCSRStatusCodeNormal` code and no reason.
 */
- (void)close;
...
/**
 The `SRWebSocketDelegate` protocol describes the methods that `ZCCSRWebSocket` objects
 call on their delegates to handle status and messsage events.
 */
@protocol ZCCSRWebSocketDelegate <NSObject>
...
/**
 ...
 @param webSocket An instance of `ZCCSRWebSocket` that received a message.
 @param message   Received message. Either a `String` or `NSData`.
 */
- (void)webSocket:(ZCCSRWebSocket *)webSocket didReceiveMessage:(id)message;
...
/**
 ...
 @param webSocket An instance of `ZCCSRWebSocket` that was open.
 */
...
/**
 ...
 @param webSocket An instance of `ZCCSRWebSocket` that failed with an error.
...
/**
 ...
 @param webSocket An instance of `ZCCSRWebSocket` that was closed.
...
/**
 ...
 @param webSocket An instance of `ZCCSRWebSocket` that received a ping frame.
 @param data      Payload that was received or `nil` if there was no payload.
 */
- (void)webSocket:(ZCCSRWebSocket *)webSocket didReceivePingWithData:(nullable NSData *)data;

/**
 Called when a pong data was received in response to ping.

 @param webSocket An instance of `ZCCSRWebSocket` that received a pong frame.
 @param pongData  Payload that was received or `nil` if there was no payload.
 */
- (void)webSocket:(ZCCSRWebSocket *)webSocket didReceivePong:(nullable NSData *)pongData;
...
/**
 ...
 @param webSocket An instance of `ZCCSRWebSocket` that received a text frame.

 @return `YES` if text frame should be converted to UTF-8 String, otherwise - `NO`. Default: `YES`.
 */
- (BOOL)webSocketShouldConvertTextFrameToString:(ZCCSRWebSocket *)webSocket NS_SWIFT_NAME(webSocketShouldConvertTextFrameToString(_:));
