...
public final class App {
    /**
     * Simple example application to save an incoming opus-encoded audio data into
     * an ogg file
     */
    private App() {
    }
    ...
    public static void main(String[] args) {
        try {
            writePreEncodedAudio();
        } catch (IOException e) {
            System.out.println("Failed to generate ogg file: " + e.getMessage());
        } finally {
            return;
        }
    }
     ...
     * @throws Exception
     */
    public static void writePreEncodedAudio() throws Exception {
        final int sampleRate = 16000;
        final int channelsCount = 1;

        Integer[][] encodedBuffers = {
            // Pre-encoded sample from https://github.com/zelloptt/zello-channel-api/blob/master/sdks/js/examples/06-send-pre-recorded.html
        ...
        File oggFile = new File("encoded.opus");
        if (oggFile.exists()) {
            oggFile.delete();
        }
        ...
        opusInfo.setSampleRate(sampleRate);
        opusInfo.setNumChannels(channelsCount);
        opusInfo.setPreSkip(120);

        OpusTags opusTags = new OpusTags();
        opusTags.addComment("title", "I'll be back from Terminator");

        OpusFile opus = new OpusFile(new FileOutputStream(oggFile), opusInfo, opusTags);
        opus.setMaxPacketsPerPage(1);

        for (Integer[] packet : encodedBuffers) {
            // Convert the data representation from integers to bytes
            byte[] data = new byte[packet.length];
            for (int i = 0; i < packet.length; i++) {
                data[i] = packet[i].byteValue();
            }
            OpusAudioData audio = new OpusAudioData(data);
            audio.setData(data);
